...
The desktop production preview, mobile source preview, and mobile production preview all render from the confirmed crop box for the selected asset. Video cropping and recropping happen directly in the desktop production preview or mobile source preview; no duplicate crop video is rendered in the settings panel. Each playback control is bound to its own visible preview video. Production toolbars show live elapsed time against the selected segment duration and animate the timeline from the video's current time. Playback seeks to the selected clip start, pauses at its end, and restarts from the segment beginning. Draft edits remain inside the crop editor until confirmed, skipped images retain their full frame, and each multi-image asset restores its own crop when selected. Preview viewports preserve the exact post-crop aspect ratio, including narrow portrait and wide landscape selections.
