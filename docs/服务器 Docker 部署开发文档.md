...
使用 Ops Control 在已配置的 Docker 服务器上创建独立的 `wottygif` Compose 项目，拉取 WottyGIF 镜像并通过 `gif.sevencn.com` 提供访问。部署不得影响服务器上已有项目。
...
4. 为 `gif.sevencn.com` 写入受控 Nginx 反向代理，执行 `nginx -t` 后再 reload。
...
- `http://gif.sevencn.com` 能访问 WottyGIF 页面或由 Cloudflare 正确转发。
